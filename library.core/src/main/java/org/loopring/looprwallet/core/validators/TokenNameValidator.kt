    ...
    override fun isValid(text: String?): Boolean {
        return when {
            text == null || text.isEmpty() -> {
                error = getTextFromResource(R.string.error_token_name_required)
                false
            }
            !text.matches(RegexUtility.NAME_REGEX) -> {
                error = getTextFromResource(R.string.error_token_name_bad_format)
                false
            }
            text.length < 2 -> {
                error = getTextFromResource(R.string.error_token_name_too_short)
                false
            }
            text.length > nameMaxLength -> {
                error = getTextFromResource(R.string.error_token_name_too_long)
                false
            }
            else -> {
                error = null
